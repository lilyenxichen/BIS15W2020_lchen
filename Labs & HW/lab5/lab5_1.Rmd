...
This is the number that R sees NAs.
...
#29 NAs in conservation are not reported in summary
```
...
#pct_miss=percentage of missing data
```
...
```{r}
vis_dat(life_history) #classes of data; most data is in numeric
```

```{r}
vis_miss(life_history) #Show percentage of missing data visually
```
