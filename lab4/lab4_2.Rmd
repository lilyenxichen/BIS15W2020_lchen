...
msleep %>% 
  filter(bodywt >= 200) %>%
  summarize(mean_sleep_lg = mean(sleep_total)) #specify column name = the function
```
...
```{r}
msleep %>% 
  count(order, sort = T) #sort = T is sorting by counts. Otherwise it will sort by abc.
```
...
```
No observation of brain weight of Cetacea. 
