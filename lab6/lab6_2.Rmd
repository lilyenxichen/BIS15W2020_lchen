...
homerange %>% 
  ...
  geom_bar() +
  labs(title = "Locomotion by Primary Method in Homerange Data",
       x = "Locomotion",
       fill = "Primary Method")
```
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.6, color = "black")+ #color puts black outline around the bins
...
homerange %>% 
  ...
  geom_histogram(alpha = 0.6, fill = "pink", color = "black")+ 
...
homerange %>% 
  ggplot(aes(x=log10.hra)) +
  geom_histogram(aes(y = ..density..), binwidth = .4, fill = "pink", alpha = 0.8, color = "black")+
...
homerange %>% 
  ...
  summarize(class_number = n_distinct(taxon))
```
...
homerange %>% 
  ggplot(aes(x = log10.mass)) +
  geom_histogram() +
  facet_grid(~thermoregulation + locomotion) #more than one categories
```
